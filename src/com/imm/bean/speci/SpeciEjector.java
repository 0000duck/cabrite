...
public class SpeciEjector implements iMode {
	...
	public void init() {
		try {
			kvCylinderDiameter = HmiVariableService.getService().getVariable("Ejector" + index + ".sv_CylinderData.rCylinderDiameter");
			kvPistonrodDiameter = HmiVariableService.getService().getVariable("Ejector" + index + ".sv_CylinderData.rPistonRodDiameter");
			kvNumCylinders = HmiVariableService.getService().getVariable("Ejector" + index + ".sv_CylinderData.iNumCylinders");
			kvUserSmallSize = HmiVariableService.getService().getVariable("Ejector" + index + ".sv_CylinderData.bUseSmallSize");
			kvMaxSpeedFwd = HmiVariableService.getService().getVariable("Ejector" + index + ".sv_rMaxSpeedFwd");
			kvMaxSpeedBwd = HmiVariableService.getService().getVariable("Ejector" + index + ".sv_rMaxSpeedBwd");
	...
	public void refresh() throws VartypeException, NetworkException, IOException {
		final KVariable vars[] = { kvCylinderDiameter, kvPistonrodDiameter, kvNumCylinders,
				kvUserSmallSize, kvMaxSpeedFwd, kvMaxSpeedBwd};
